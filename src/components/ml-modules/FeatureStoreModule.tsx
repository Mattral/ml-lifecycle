...
const FeatureStoreModule: React.FC<FeatureStoreModuleProps> = ({ onComplete }) => {
  useMLPipeline(); // validate context availability
  ...
  const [servingResult, setServingResult] = useState<ServingResult | null>(null);
  ...
  const filteredFeatures = useMemo(() =>
    ...
    [features, searchQuery, filterType, filterStatus],
  );
  ...
  }, [newFeatureName, newFeatureType, newFeatureSource, newFeatureDesc]);

  const bumpVersion = useCallback((id: string) => {
    setFeatures((prev) => prev.map((f) => (f.id === id ? { ...f, version: f.version + 1 } : f)));
  }, []);

  const simulateServing = useCallback(async () => {
    ...
    for (const f of activeFeatures) {
      if (f.type === 'numerical') result[f.name] = (Math.random() * 100).toFixed(2);
      else if (f.type === 'categorical') result[f.name] = 'category_A';
      else if (f.type === 'embedding') result[f.name] = '[0.12, -0.34, ...]';
      else result[f.name] = new Date().toISOString();
    }
  ...
  const getTypeBadgeVariant = (type: Feature['type']): 'default' | 'secondary' | 'outline' => {
    switch (type) {
      ...
      <div className="grid lg:grid-cols-3 gap-6">
              ...
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div>
                  <CardTitle className="text-base flex items-center gap-2">
                    <Database className="w-4 h-4" aria-hidden="true" /> Feature Registry
                  </CardTitle>
                  <CardDescription>{filteredFeatures.length} features</CardDescription>
                </div>
                <Button size="sm" variant="outline" onClick={() => setShowAddForm(!showAddForm)}>
                  <Plus className="w-4 h-4 mr-1" aria-hidden="true" /> Register Feature
                </Button>
              </div>
                  ...
                  <TableBody>
                    {filteredFeatures.map((f) => (
                      <TableRow key={f.id} className={`cursor-pointer ${selectedFeature?.id === f.id ? 'bg-primary/5' : ''}`} onClick={() => setSelectedFeature(f)}>
                        ...
                        <TableCell className="text-xs"><div className="flex items-center gap-1"><Clock className="w-3 h-3" aria-hidden="true" /> {f.freshness}</div></TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          <Button variant="ghost" size="sm" className="h-6 text-[10px]" onClick={(e) => { e.stopPropagation(); bumpVersion(f.id); }} aria-label={`Bump version for ${f.name}`}>
                            <Tag className="w-3 h-3 mr-1" aria-hidden="true" /> Bump
                          </Button>
                        </TableCell>
                      </TableRow>
                    ))}
        ...
        <div className="space-y-4">
          ...
          <Card className="glass-card">
            <CardHeader className="pb-3">
              <CardTitle className="text-base flex items-center gap-2">
                <Zap className="w-4 h-4" aria-hidden="true" /> Online Serving
              </CardTitle>
            ...
            <CardContent className="space-y-3">
              <Button size="sm" className="w-full" onClick={simulateServing} disabled={servingSimulating}>
                {servingSimulating ? (
                  <><RefreshCw className="w-4 h-4 mr-1 animate-spin" aria-hidden="true" /> Fetching...</>
                ) : (
                  <><Zap className="w-4 h-4 mr-1" aria-hidden="true" /> Fetch Feature Vector</>
                )}
              </Button>
              {servingResult && (
      ...
      {features.length > 0 && (
        <div className="flex justify-end">
          <Button onClick={handleComplete} disabled={moduleComplete}>
            {moduleComplete ? (
              <><CheckCircle className="w-4 h-4 mr-2" aria-hidden="true" /> Feature Store Complete</>
            ) : (
